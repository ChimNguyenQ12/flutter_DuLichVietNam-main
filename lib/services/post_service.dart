...
//create post
Future<ApiResponse> createPost(int placeid, String body, String? image) async {
  ApiResponse apiResponse = ApiResponse();
  try {
    String token = await getToken();
    final response = await http.post(Uri.parse(postsURL),
        ...
        body: image != null
            ? {'place_id': '$placeid', 'body': body, 'image': image}
            : {'place_id': '$placeid', 'body': body});
...
Future<ApiResponse> deletePost(int postId) async {
  ...
  try {
    String token = await getToken();
    final response = await http.put(Uri.parse('$postsdelURL/$postId'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        });
